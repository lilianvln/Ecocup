        ...
        for i in range(len(image_list)):
            ...
            if img != '.DS_Store':
                ...
                if boxes is not None and boxes.any():
                    ...
                    scale_factor = 1.0
                    while scale_factor > 0.1 and img.shape[0] > self.window_size[0] and img.shape[1] > self.window_size[
                        1]:
                ...
                while scale_factor > 0.1 and img.shape[0] > self.window_size[0] and img.shape[1] > self.window_size[1]:
                    for y in range(0, img.shape[0] - self.window_size[0], self.stride):
                        for x in range(0, img.shape[1] - self.window_size[1], self.stride):
                            ...
                            resized_window = resize(window, self.window_size)
                            # Extraire les descripteurs HOG de la région
                            hog_features = hog(rgb2gray(resized_window), orientations=9, pixels_per_cell=(8, 8), cells_per_block=(3, 3), block_norm='L2-Hys', feature_vector=True)
                            # Ajouter les descripteurs HOG à la liste des exemples négatifs
...
def ajouter_cadre(numero_image, coordonnee_ligne, coordonne_colonne, hauteur, largeur, score):
    # Charger l'image
...
classifier = Classifier(window_size=(64, 64), stride=100, threshold=0.5)
classifier.fit()
...
for i in predict:
    if i != []:
        ...
        largeur = int(i[0][4])
        score = i[0][5]
        ajouter_cadre(numero_image, coordonnee_ligne, coordonne_colonne, hauteur, largeur, score)
